"""Monitoring and metrics collection for the MCP Todo Server"""
import time
...
def get_mcp_metrics() -> Dict[str, Any]:
    ...
    return {
        'operations_total': metrics_collector.get_counter_value('mcp_operations_total'),
        'errors_total': metrics_collector.get_counter_value('mcp_errors_total'),
        'avg_operation_time': get_average_operation_time('mcp_operation_time'),
    }


def get_average_operation_time(metric_name: str) -> Optional[float]:
    """Calculate average operation time for a timer metric"""
...
def increment_operations():
    """Increment MCP operations counter"""
    metrics_collector.increment_counter('mcp_operations_total')


def increment_errors():
    """Increment MCP errors counter"""
    metrics_collector.increment_counter('mcp_errors_total')


def record_operation_time(duration: float, operation: str = None):
    """Record MCP operation time"""
    tags = {'operation': operation} if operation else {}
    metrics_collector.record_metric('mcp_operation_time', duration, tags, 'timer')