...
### Core Policy: Default to Main

**Unless the user EXPLICITLY requested a feature branch, you must commit directly to `main`.**
...
- Did user explicitly say "create feature branch", "new branch", or "start PR"?
  - **YES** → Use **Strategy B (Feature Branch)**.
  - **NO** → Use **Strategy A (Direct Commit to Main)**.

**2. Execute Selected Strategy:**

**Strategy A: Direct Commit to Main (DEFAULT)**
_Used when user did NOT ask for a branch. You MUST target `main`._

1. Checks for changes (`git status`).
2. **CRITICAL:** If current branch is NOT `main`:
   - Rename it to main: `git branch -m main` (if starting fresh)
...
```bash
git add .
git checkout -b <branch-name>   # or git switch
git commit -m "conventional commit message"
